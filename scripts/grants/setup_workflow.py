#!/usr/bin/env python3
"""Deploy OK grant program officer email workflow + Agent Builder workflow tool."""
...
YAML_PATH = Path(__file__).resolve().parent / "grant-program-email-draft.yaml"
WORKFLOW_ID = "ok-grant-program-officer-email"
TOOL_ID = "ok-grants-program-email-workflow"
...
def upsert_workflow(kb: str, api_key: str, yaml: str) -> None:
    try:
        ...
        request(
            ...
            api_key,
            "POST",
            {"workflows": [{"yaml": yaml, "name": "OK Grant Program Officer Email"}]},
        )
...
def upsert_workflow_tool(kb: str, api_key: str) -> None:
    create_payload = {
        "id": TOOL_ID,
        "type": "workflow",
        "description": "Draft program-officer outreach email for a grant awardee using ok-grant-data context.",
