#!/usr/bin/env python3
"""Deploy Texas College alumni email workflow + Agent Builder workflow tool."""
...
YAML_PATH = Path(__file__).resolve().parent / "booster-alumni-email-draft.yaml"
WORKFLOW_ID = "texas-college-alumni-outreach-email"
TOOL_ID = "booster-alumni-email-workflow"
...
def upsert_workflow(kb: str, api_key: str, yaml: str) -> None:
    try:
        ...
        request(
            ...
            api_key,
            "POST",
            {"workflows": [{"yaml": yaml, "name": "Texas College Alumni Outreach Email"}]},
        )
...
def upsert_workflow_tool(kb: str, api_key: str) -> None:
    create_payload = {
        "id": TOOL_ID,
        "type": "workflow",
        "description": "Draft personalized alumni outreach email for a Texas College athletic booster donor.",
